...
The data is read using [readModBam]{.fnio} (`readFunction` argument).
...
#| eval: !expr identical(Sys.getenv("RECOMPUTE"), "1")
...
dmrL <- lapply(seq_along(chrlens), function(i) {
    scanForHighScoringRegions(
        bamfiles = bamfiles,
        readFunction = "readModBam",
        readFunctionArgs = list(
...
First, we call the [quantifyWindowsInRegion]{.fn} function.
Since our data is stored in a modBam file, we use [readModBam]{.fnio} as the _readFunction_ with appropriate arguments in _readFunctionArgs_.
We use [sumNmodNvalid]{.fn} as the _quantFunction_, to get the total and modified nucleotide and the fraction of modified nucleotides for each window.
The windows will be automatically defined within the specified `regions` (here a single interval), given the indicated window size and step. 
...
```{r}
#| label: save-cpg-methylation-scanning
#| eval: !expr identical(Sys.getenv("RECOMPUTE"), "1")
#| echo: false
...
saveRDS(rescpgL, "data/out/scanning_cpg-list.rds")
```

```{r}
#| label: load-cpg-methylation-scanning
#| echo: false

rescpgL <- readRDS("data/out/scanning_cpg-list.rds")
```
