...
We use the [readModBam]{.fnio} function to read data from an 800-bp region on chromosome 8 and store it in a `SummarizedExperiment` object. 
...
This can be done conveniently using [subsetReads]{.fnio}, which allows you to subset the object keeping either a pre-specified set of reads or a random subset. 
...
Summary-level assays can be generated from read-level values using, e.g., the [flattenReadLevelAssay]{.fnio} function. 
...
## Example: A transcription factor binding site

Most of the data that is used for illustration in this book has been generated on an Oxford Nanopore (ONT) sequencer, but `modBam` files are standardized and [fmicompbio/SingleMoleculeGenomicsIO]{.githubpkg} and [fmicompbio/footprintR]{.githubpkg} are agnostic to the sequencing technology used to generate them.
In this example, we will visualize a single locus around a bound CTCF site for ONT- and a PacBio-generated datasets:

```{r}
#| fig.width: 10
#| fig.height: 5
#| label: fig-ont-vs-pacbio

# Read data
ctcfsite <- as("chr8:6622465-6622483", "GRanges")
reg <- resize(ctcfsite, width = 2001, fix = "center")

se <- readModBam(
    bamfiles = c(ONT = "data/mESC_wt_6mA_rep1.bam",
                 PacBio = "data/mESC_wt_6mA_5mCG_PacBio_rep1.bam"),
    modbase = "a", 
    regions = reg,
    seqinfo = seqinfo(gnm), 
    sequenceContextWidth = 1, 
...
# plot
trackdef <- list(
    list(trackType = "Heatmap", trackData = "mod_prob", interpolate = TRUE,
         orderReads = "regionAvg", orderRegion = ctcfsite + 30,
         orderReverse = TRUE, legendTitle = "6mA"),
    list(trackType = "Smooth", trackData = "FracMod",
         smoothMethod = "rollingMean", windowSize = 21,
         arglistSmooth = list(linewidth = 1),
         highlightRegions = ctcfsite, showLegend = FALSE))

wrap_plots(
    plotRegion(
        se[, "ONT"], region = reg, minCoveredFraction = 0.8,
        tracks = trackdef, 
        sequenceContext = "A", referenceCoordinate = mid(ctcfsite)
    ) + plot_layout(heights = c(3, 1)),
    plotRegion(
        se[, "PacBio"], region = reg, minCoveredFraction = 0.8,
        tracks = trackdef, 
        sequenceContext = "A", referenceCoordinate = mid(ctcfsite)
    ) + plot_layout(heights = c(3, 1)),
